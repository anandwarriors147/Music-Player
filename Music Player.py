...
btn_report = Button(root, bg='white', fg='#4d0000', text='Favourite Songs', font=('Courier', 14, 'bold'))
btn_report.place(x=150, y=520)
...
'''
Used to generate the report of top three favourite songs.
'''


def generate_report(event):
	mycol.insert(report)
	retrieved_data = dict(mycol.find_one())
	retrieved_data.pop('_id', None)
	
	top_n = (nlargest(3, retrieved_data, key=retrieved_data.get))
	freq = []
	for song_name in top_n:
		freq.append(retrieved_data[song_name])
	
	y_pos = np.arange(len(top_n))
	mp.bar(y_pos, freq, align='center', alpha=0.5, color=['red', 'green', 'cyan'], edgecolor='#ff8400')
	mp.xticks(y_pos, top_n)
	mp.ylabel('Number of times played')
	mp.xlabel('Song name')
	mp.title("Top 3 Favourite Songs")
	mp.show()
...
btn_report.bind('<Button-1>', generate_report)
